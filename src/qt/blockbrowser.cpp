...
void BlockBrowser::updateExplorer(bool block)
{
    if(block)
    {
        ...
        ui->pawBox->setText(QPawrate + " Hash/sec");
    }
    ...
    if(block == false) {
        ...
        QString QValue;
        if (value==-1) QValue = "Reward";
        else QValue = QString::number(value, 'f', 6);
        ...
        ui->valueBox->setText(QValue + " BERN");
        ui->txID->setText(QID);
        ui->outputBox->setText(QOutputs);
        ui->inputBox->setText(QInputs);
        ui->feesBox->setText(QFees + " BERN");
    }
}
