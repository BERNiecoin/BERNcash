...
{
    resize(930, 550);
    setWindowTitle(tr("BERN") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    ...
    setStyleSheet("QToolTip { color: #cecece; background-color: #333333;  border:0px;} ");
    ...
    labelMiningIcon->setObjectName("labelMiningIcon");
    ...
    labelMiningIcon->setStyleSheet("#labelMiningIcon QToolTip {color:#cecece;background-color:#333333;border:0px;}");
    ...
    timerMintingWeights->start(30 * 100); // was 1000
    connect(timerMintingWeights, SIGNAL(timeout()), this, SLOT(updateMintingWeights()));
    // Set initial values for user and network weights
    nWeight = 0;
    nNetworkWeight = 0;
    // Set mining pixmap
    labelMiningIcon->setPixmap(QIcon(":/icons/powmining_inactive").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
    ...
    statusBar()->setObjectName("BERNstatusBar");
    statusBar()->setStyleSheet("#BERNstatusBar { border-top-color: qlineargradient(spread:pad, x1:0, y1:0, x2:0, y2:1, stop:0 #4B4F52, stop:0.5 #8B8F92, stop:1.0 #8B8F92); border-top-width: 2px; border-top-style: inset; background-color: qlineargradient(spread:pad, x1:0, y1:0, x2:0, y2:1, stop:0 #666666, stop:0.5 #323232, stop:1.0 #111111);  color: #ffffff; } QToolTip { color: #ffffff; background-color: #9D0000; border-width: 1px;border-color:#CA0D0D;}");  
...
void BitcoinGUI::createActions()
{
    ...
    overviewAction->setToolTip(tr("Show general overview of BERN wallet"));
    overviewAction->setCheckable(true);
    overviewAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_1));
    tabGroup->addAction(overviewAction);

    sendCoinsAction = new QAction(QIcon(":/icons/send"), tr("&Send"), this);
    sendCoinsAction->setToolTip(tr("Send to a BERN address"));
    sendCoinsAction->setCheckable(true);
    sendCoinsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_2));
    tabGroup->addAction(sendCoinsAction);

    receiveCoinsAction = new QAction(QIcon(":/icons/receiving_addresses"), tr("&Receive"), this);
    receiveCoinsAction->setToolTip(tr("Show the list of your BERN addresses for receiving payments"));
    receiveCoinsAction->setCheckable(true);
    receiveCoinsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_3));
    tabGroup->addAction(receiveCoinsAction);

    historyAction = new QAction(QIcon(":/icons/history"), tr("&Ledger"), this);
    historyAction->setToolTip(tr("Browse your BERN transaction history"));
    historyAction->setCheckable(true);
    historyAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_4));
    tabGroup->addAction(historyAction);

    addressBookAction = new QAction(QIcon(":/icons/address-book"), tr("&Addresses"), this);
    addressBookAction->setToolTip(tr("Edit the list of your stored BERN addresses and labels"));
    addressBookAction->setCheckable(true);
    addressBookAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
    tabGroup->addAction(addressBookAction);

    miningAction = new QAction(QIcon(":/icons/mining"), tr("&Mine"), this);
    miningAction->setToolTip(tr("BERN PoW mining"));
    miningAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    miningAction->setCheckable(true);
    tabGroup->addAction(miningAction);

    statisticsAction = new QAction(QIcon(":/icons/statistics"), tr("&Stats"), this);
    statisticsAction->setToolTip(tr("BERN statistics"));
    statisticsAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_7));
    statisticsAction->setCheckable(true);
    tabGroup->addAction(statisticsAction);

    blockAction = new QAction(QIcon(":/icons/block"), tr("&Explore"), this);
    blockAction->setToolTip(tr("Explore the BERN blockchain"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About BERN"), this);
    aboutAction->setToolTip(tr("Show information about BERN"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for BERN"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("BERN Wallet") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("BERN Wallet"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to the BERN network.", "", count));
}
...
void BitcoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
    ...
    //tooltip = tr("Current PoW difficulty is %1.").arg(clientModel->GetDifficulty(GetLastBlockIndex(pindexBest, true)) + QString("<br>") + tooltip;
    //tooltip = tr("Current PoS difficulty is %1.").arg(clientModel->GetDifficulty()) + QString("<br>") + tooltip;
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid BERN address or malformed URI parameters."));
    }
...
void BitcoinGUI::setMining(bool mining, int hashrate)
{
    if (mining)
    {
        labelMiningIcon->setPixmap(QIcon(":/icons/powmining_active").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
        labelMiningIcon->setToolTip(tr("Mining BERN at %1 hashes per second").arg(hashrate));
    }
    else
    {
        labelMiningIcon->setPixmap(QIcon(":/icons/powmining_inactive").pixmap(STATUSBAR_ICONSIZE,STATUSBAR_ICONSIZE));
        labelMiningIcon->setToolTip(tr("Not mining BERN"));
    }
}
